...
export default function DashboardPage() {
  ...
  const fetchSellerProducts = useCallback(async () => {
    const sellerId = localStorage.getItem("warpzone-user-id") || userId
    if (!sellerId) {
      console.log("No sellerId available from localStorage or context")
      return
    }
    console.log("Fetching products for sellerId:", sellerId)
    setIsLoadingProducts(true)
    try {
      const response = await fetch(`/api/products?sellerId=${sellerId}&showAll=true`)
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-4xl font-black mt-2">{sellerProducts.length}</p>
                ...
                {isLoadingProducts ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {sellerProducts.map((product) => (
                      <Card
                        key={product.id}
                        className={`bg-white shadow-md hover:shadow-lg transition-shadow ${product.approval_status === "pending" ? "opacity-60" : ""
                          }`}
                      >
                        <CardContent className="p-6">
